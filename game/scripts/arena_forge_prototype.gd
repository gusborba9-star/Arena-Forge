...
func _ready() -> void:
    ...
    match_state.configure(90.0, 180.0, 240.0)
    ...
    director.configure(ArenaRuntimeDefinitions.arena_1())
    var cards: Array[ArenaCard] = []
    for i in range(8): cards.append(ArenaCard.from_definition(CardDefinitions.initial()[i]))
    deck.set_deck(cards)
...
func _process(delta: float) -> void:
    if hero.dead or match_state.is_result():
        ...
        queue_redraw(); return
    match_state.tick(delta); energy.tick(delta); runtime.tick(delta); director.tick(delta)
    ...
    if attack_timer<=0.0: _auto_attack(); attack_timer=0.55
    if enemy_timer<=0.0: _enemy_attack(); enemy_timer=1.2
...
func _tick_event(delta: float) -> void:
    if pending_event.is_empty() and not telegraph.active:
        ...
        if not pending_event.is_empty(): telegraph=Telegraph.new(str(pending_event.id),float(pending_event.get("warning_seconds",1.5)))
...
func _move_hero(delta: float) -> void:
    var d:=input.get_move_vector()
    if d==Vector2.ZERO:
        d=Input.get_vector("ui_left","ui_right","ui_up","ui_down")
    hero.move(d,delta); hero.position.x=clampf(hero.position.x,70,1210); hero.position.y=clampf(hero.position.y,70,650)
...
func _enemy_attack()->void:
    for e in enemies:
        if e.dead:continue
        var r:=280.0 if e.role==ArenaEnemy.Role.RANGED else 42.0
...
func _draw()->void:
    ...
    if pending_upgrade:
        draw_string(ThemeDB.fallback_font,Vector2(420,200),"ESCOLHA UMA MELHORIA: 1  2  3",HORIZONTAL_ALIGNMENT_LEFT,-1,26)
